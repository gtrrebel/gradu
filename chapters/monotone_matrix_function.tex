...
\begin{proof}
	...
	We may also assume that if $B - A = c P_{w}$, then $w$ is not orthogonal to any eigenvector of $A$. Indeed, if this would be the case, we could decompose $V = \vspan\{v_{1}\} \oplus V'$, where $v_{1}$ is the eigenvector, and factorize $A = \arestr{A}{\vspan\{v_{1}\}} \oplus \arestr{A}{V'}$ and $P_{w} = 0 \oplus \arestr{(P_{w})}{V'}$. But now checking that $f(B) - f(A) \geq 0$ boils down to checkin that $f(\arestr{B}{V'}) - f(\arestr{A}{V'}) \geq 0$, which would follow if we could prove that $f \in P_{n - 1}(a, b)$. But this follows if we induct on $n$ in the main statement and use lemma \ref{k_tone_cor}.

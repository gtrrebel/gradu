...
\begin{enumerate}
	\item Canonicality of compression
	\item Eigenvalues of $AB$ when $A$ and $B$ are positive
	\item Symmetric product fail
	\item General Sylvester's law of inertia
	\item Bernstein's big theorem
	\item Regularity of matrix functions
	\item $2 \times 2$ Pick functions
	\item Hindmarsh theorem
	\item $c_{0}$ for Pick functions
	\item From Pick point to matrices
	\item From Pick points to weakly Pick
	\item Finite Pick extension lemma
	\item Monotonicity and derivative
	\item smooth Dobsch char.

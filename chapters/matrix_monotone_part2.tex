...
This is the correct interpretation and it makes sense without further regularity assumptions. There are also many different ways to talk about the polynomial part.
...
Theorem \ref{main_theorem} gives also a resolution to the regularity issues of its predecessor, theorem \ref{heuristic_loewner}. Recall that $(2 n - 1)$-tone functions are $C^{2 n - 3}$ and their $(2 n - 3)$'th derivative is convex. As convex functions are twice differentiable almost everywhere, $f N(q)$ is $(2 n - 1)$ times differentiable almost everywhere.
...
\begin{align*}
	...
	\Leftrightarrow &[\lambda_{1}, \lambda_{1}, \ldots, \lambda_{n}, \lambda_{n}]_{f N(q)} \text{ for any $q \in \C_{n - 1}[x]$ and $a < \lambda_{1} < \ldots < \lambda_{n} < b$}.
\end{align*}
...
\end{kor}
\begin{proof}
	t.f.i.f \ref{main_theorem} and \ref{k_tone_local}.
\end{proof}
...
\begin{proof}[Proof of lemma \ref{projection_eigenvalues}]
	...
	\end{align*}
	It has $n$ poles of negative residue so it has a root between any two poles. Also it tends to $1$ at infinity so it has a root on $(\lambda_{1}(A), \infty)$. It has hence $n$ distinct roots. All these roots are eigenvalues of $B$ so they are exactly the eigenvalues. This implies the first claim.

	For the converse, first take $A$ with the given eigenvalues. By the previous lemma we now just want to choose $v$ in such a way that
	\begin{align*}
...
\begin{proof}[Proof of lemma \ref{main_lemma}]
	...
	\begin{align*}
		\det(z I - B) \langle (z I - B)^{-1} v, w \rangle = \overline{\det(\overline{z} I - A) \langle (\overline{z} I - A)^{-1} w, v \rangle}
	\end{align*}
	so if we write $q(z) = \det(z I - A) \langle (z I - A)^{-1} w, v \rangle$, we have
	\begin{align*}
		\langle (f(B) - f(A)) w, w \rangle = [\lambda_{1}(A), \ldots, \lambda_{n}(A), \lambda_{1}(B), \ldots, \lambda_{n}(B)]_{f N(q)}.
	\end{align*}
	By \ref{entire_interpolation} or by interpreting everything formally one sees that this identity holds without any regularity assumptions.
...
\begin{lem}\label{k_tone_cor}
	If $n \geq 1$ and $f N(q)$ is $(2 n + 1)$-tone for $q \in \C_{n}[x]$, then $f N(\tilde{q})$ is $(2 n - 1)$-tone for every $\tilde{q} \in \C_{n - 1}[x]$.
\end{lem}
...
\begin{proof}[Proof lemma \ref{k_tone_cor}]
	Take any $\tilde{q} \in R_{n}(a, b)$. If there was no condition on the order of the poles, we could simply note that
	\begin{align*}
		\tilde{q} = \tilde{q} \frac{N(z - c)}{(z - c) (z - c)} \in R_{n + 1}(a, b).
	\end{align*}
	Even though that doesn't quite work, we can modify the idea a little: we have
	\begin{align*}
		\tilde{q} = \frac{1}{2} \tilde{q} \frac{N((z - c))}{(z - c) (z - \frac{c + d}{2})} +  \frac{1}{2} \tilde{q}\frac{N((z - d))}{(z - d) (z - \frac{c + d}{2})} \in \cone(R_{n + 1}(a, b))
	\end{align*}
...
\begin{lause}\label{integral_loewner}[Loewner's theorem, version $1$]

...
\begin{proof}
	...
	To prove the claim note that if $A$ has $k$ non-negative eigenvalues, by lemma \ref{subspace_lemma} it's restriction to some $k$-dimensional subspace is positive. But then also the restriction of $B$ to this subspace is positive, so also $B$ has at least $k$ non-negative eigenvalues.
\end{proof}
...
Eigenvalues work also desirably with restriction.
...
\begin{proof}
	...
	To prove the claim, note again that if $\arestr{A}{W}$ has $k$ non-negative eigenvalues, by lemma \ref{subspace_lemma} it's restriction to some $k$-dimensional subspace is positive. But then also restriction of $A$ to this same subspace is positive and hence it has $k$ non-negative eigenvalues.
\end{proof}

Again one can prove that this result is strongest possible, but the proof is lot more involved.

\begin{prop}
	For any $a_{1} \geq b_{1} \geq a_{2} \geq \ldots b_{n - 1} \geq a_{n}$ we may find $A \in \H^{n}(V)$ with $a_{i}$'s as spectra and $(n - 1)$-dimensional subspace $W$ of $V$ such that eigenvalues of $\arestr{A}{W}$ are the $b_{i}$'s.
\end{prop}

...
\begin{kor}\label{projection_eigenvalues}
	Let $A \leq B$ such that $B - A = v v^{*}$ for some $v \in V$. Then
	\begin{align*}
...
\begin{prop}\label{projection_eigenvalues_con}
	For any $b_{1} \geq a_{1} \geq b_{2} \geq a_{2} \ldots \geq b_{n} \geq a_{n}$ we may find $A, B \in \H^{n}(V)$ with $a_{i}$'s and $b_{i}$'s as spectra of $A$ and $B$, respectively and $B - A = v v^{*}$ for some $v \in V$.
\end{prop}
...
\begin{lem}\label{projection_characteristic_polynomial}
	Let $A, B \in \H$ be such that $B - A = v v^{*}$ for some $v \in V$. Then
	\begin{align*}

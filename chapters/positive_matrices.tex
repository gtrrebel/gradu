...
\section{Restricting linear maps}

There's however quite clever way to go around this. Instead of requiring non-negativity of eigenvalues, we require that map ``restricts" to positive map. The idea is: if we already know which maps should be positive in one-dimensional spaces, we should require that when we restrict our look to one-dimensional subspaces, we should get a positive map.
...
\begin{itemize}
	\item Restriction of $A \in \L(V)$ to subspace $W$, which we will \textit{$*$-restriction} and denote by $\arestr{A}{W}$, should be a map in $\L(W)$.
	\item If $A(W) \subset W$, restriction should coincide with the original map, in the sense that $\arestr{A}{W}w = Aw$ for any $w \in W$.
	\item Restriction operation is linear (in the linear map).
	\item If $W' \subset W \subset V$, we should have $\arestr{A}{W'} = \arestr{(\arestr{A}{W})}{W'}$.
\end{itemize}

These properties don't uniquely define a linear map but they say that $\arestr{A}{W}$ should be of the form $P_{V, W} \circ A \circ \incl{V}{W}$ where $P_{V, W}$ is a projection, i.e. a idemponent linear map with $V \to W$ with image $W$. Moreover, these projections should satisfy $P_{V, W'} = P_{W, W'} \circ P_{V, W}$.
...
\begin{prop}
	...
	\begin{enumerate}[(i)]
		\item If $W \subset V$ and $A \in \L(V)$ such that $A(V) \subset W$, then $\arestr{A}{W}w = Aw$ for any $w \in W$.
		\item If $W' \subset W \subset V$ and $A \in \L(V)$, we have $\arestr{A}{W'} = \arestr{(\arestr{A}{W})}{W'}$.
		\item For any $W \subset V$ and $A, B \in \L(V)$ and $\alpha, \beta \in \R$ we have $\arestr{\alpha A + \beta B}{W} = \alpha \arestr{A}{W} + \beta \arestr{B}{W}$.
	\end{enumerate}
\end{prop}

We implicitly defined restriction for inner product spaces over $\R$, but such definition could be done just as well for $\C$. In many ways this is more natural space as we'll soon see.

\section{Positive maps}

Now that we have defined the restrictions we are ready define positive maps.
...
\begin{prop}
	...
	\begin{enumerate}[(i)]
		...
		\item $A \in \L(V)$, $A$ is positive if and only for any $v \in V$ we have $\langle A x, x \rangle \geq 0$.

...
``$A$ is positive" is denoted by $A \geq 0$, and ``$A$ is stricly positive" by $A > 0$. The set of positive maps is denoted by $\H_{+}(V)$, shortened to $\H_{+}$. Here is a big list of important facts and equivalent definitions for the previous.
...
There is one very important way to produce positive maps from others, called congruence. Given any two positive maps $A$ and $B$, their composition need not be positive, but the map $B^{*}AB$ is. First of all, it is self-adjoint, as $(B^{*}AB)^{*} = B^{*} A B^{**} = B^{*}AB$. Also $Q_{B^{*}AB}(v) = \langle B^{*}AB v, v \rangle = \langle A (B v), (B v) \rangle \geq 0$ for any $v \in V$. We didn't really need the assumption on the positivity of $B$, so it could have been any linear map. We also see that $Q_{B^{*}AB} = Q_{A} \circ B$, which also explains the positivity. Also, the construction makes sense for any map $A$. If $A, B, C \in \L(V)$ and $C = B^{*}AB$ for some invertible $B$, we say that $A$ and $C$ are congruent. It is easily verified that congruence is a equivalence relation.

The previous construction can be also performed between two spaces $V$ and $W$: given any map $B \in \L(V, W)$ and $A \in \H_{+}(W)$, we note that $B^{*}AB \in \H_{+}(V)$. We see that self-adjointness is preserved in congruences, but more generally, if $A \in \normal(V)$, $B^{*}AB$ need not be normal for every $B \in \L(V)$. For self-adjoint maps we can say a lot more: while congruence doesn't in general preserve eigenvalues, it preserves their signs.
...
\end{lause}
\begin{proof}
	Let's start with the ``if" part. Let's denote the eigenvalues of $A$ and $B$ by $\lambda_{1} \leq \lambda _{2} \leq \ldots \leq \lambda_{n}$ and $\lambda_{1}' \leq \lambda _{2}' \leq \ldots \leq \lambda_{n}'$, respectively, and the corresponding eigenvectors with $v_{1}, v_{2}, \ldots, v_{n}$ and $v_{1}', v_{2}', \ldots, v_{n}'$. By assumption $\lambda_{i}$ and $\lambda_{i}'$ have the same sign (or are both zero) for any $1 \leq i \leq n$, so we may find real numbers $t_{1}, t_{2}, \ldots, t_{n}$ such that $\lambda_{i} = \lambda_{i}' t_{i}^{2}$. Now consider a linear map $C$ with $C v_{i} = t_{i} v_{i}'$. Now $(Q_{B} \circ C)(v) = Q_{B}(\sum_{i = 1}^{n} \langle v, v_{i} \rangle t_{i} v_{i}') = \sum_{i = 1}^{n} |\langle v, v_{i} \rangle|^{2} t_{i}^2 \lambda_{i}' = \sum_{i = 1}^{n} |\langle v, v_{i} \rangle|^{2} \lambda_{i} = Q_{A}(v)$ so $Q_{C^{*}BC} = Q_{B} \circ C = Q_{A}$. Hence $A$ and $B$ are congruent.

	The ``only if" - part is a bit trickier. TODO
\end{proof}


If two normal maps are congruent, however, we can vastly generalize the previous result on positive maps: for normal maps, congruence preserves eigenvalues on any open ray, so in particular also rank.

For any $A \in \normal(V)$ define the mapping $\theta[A] : \C \to \N$ by setting $\theta[A](z)$ be the number of eigenvalues (counted with multiplicity) on ray $\{rz | r > 0\}$. This map is called the \textit{angularity} of $A$.

\begin{lause}[Generalized Sylvester's Law of Inertia]
	If $A, B \in \normal(V)$ are congruent, then $\theta[A] = \theta[B]$.
\end{lause}

This surprising theorem follows from the following lemma.

\begin{lem}
	If $A, B \in \normal(V)$ are congruent, then $A$ and $B$ have equally many eigenvalues on the closed right half-plane $\{z \in \C | \Re(z) \geq 0\}$, i.e. with non-negative real part.
\end{lem}
\begin{proof}
	TODO
\end{proof}

\begin{proof}[Proof of the Sylvester's law of inertia]
	TODO
\end{proof}

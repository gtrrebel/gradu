...
\end{align}
\textit{Woodbury matrix identity}. Why might such identity be useful? The idea is that if $\dim(W_{2}) \ll \dim(W_{1})$, the identity is way to connect inverse of $A - B C^{-1} B^{*}$, low rank update of $A$, and $A$. If $\dim(W_{2}) = 1$ for instance, by setting $C = -\frac{1}{c}$ for some $c > 0$ and $B = v$ for some $v \in V$ with $|v| = 1$ we get
\begin{align*}
	A^{-1} - \frac{A^{-1} P_{v} A^{-1}}{\frac{1}{c} + \langle A^{-1}v, v\rangle} = (A + c P_{v})^{-1}:
\end{align*}
inverse of rank 1 update can be easily calculated if one knows the inverse of the original map.

In a similar vein one obtains formulas for determinants. By comparing determinants from two LDL-decompositions we arrive at
\begin{align}\label{matrix_determinant_lemma}
	\det(A) \det(C - B^{*} A^{-1} B) = \det(C) \det(A - B C^{-1} B^{*}),
\end{align}
\textit{matrix determinant lemma}. Again, by the previous choices for $B$ and $C$ we arrive at
\begin{align*}
	\det(A) \left(1 + c \langle A^{-1}v, v\rangle\right) = \det(A + c P_{v}):
\end{align*}
...
TODO prove the 1d-projection case with the previous restricting to orthocomplement of the projection.

TODO picture of eigenvalues changing when compressed

Again one can prove that this result is strongest possible, but the proof is lot more involved.
...
\begin{kor}
	Let $A \leq B$ such that $B - A = c P_{v}$ for some one-dimensional projection and $c \geq 0$. Then
	\begin{align*}
...
\begin{prop}\label{projection_eigenvalues_con}
	For any $b_{1} \geq a_{1} \geq b_{2} \geq a_{2} \ldots \geq b_{n} \geq a_{n}$ we may find $A, B \in \H^{n}(V)$ with $a_{i}$'s and $b_{i}$'s as spectra of $A$ and $B$, respectively and $B - A = c P_{v}$ for some $v \in V$ and $c \geq 0$.
\end{prop}
...
\begin{lem}
	Let $A, B \in \H$ be such that $B - A = c P_{v}$ for some $c \in \R$ and $v \in V$ with $|v| = 1$. Then
	\begin{align*}
		\det(B - z I) = \det(A - z I) \left(1 + c \langle (A - z I)^{-1}v, v\rangle\right).
	\end{align*}
...
\begin{proof}[Proof of propostion \ref{projection_eigenvalues_con}]
	If $a_{i} = b_{j}$ for some $1 \leq i, j \leq n$ we can forget $a_{i}$ and $b_{j}$, solve the remaining problem on smaller space to get $A'$ and $v'$ and take $A : V' \oplus \C \to V' \oplus \C$ to be $A' \oplus 0$. We may hence assume that the numbers are distinct.

	First take $A$ with the given eigenvalues. By the previous lemma we just want to choose $c$ and $v$ in such a way that
	\begin{align*}
		\frac{p_{B}(z)}{p_{A}(z)} = 1 + c \langle (A - z I)^{-1}v, v\rangle= 1 + c \sum_{i = 1}^{n} \frac{|\langle v, e_{i} \rangle|^2}{a_{i} - z},
	\end{align*}
	...
	\begin{align*}
		z \mapsto 1 + c \sum_{i = 1}^{n} \frac{|\langle v, e_{i} \rangle|^2}{a_{i} - z}
	\end{align*}

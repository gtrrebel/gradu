...
Also, given $A \in \H(V)$, we may write any $x \in V$ in the form $v = \sum_{1 \leq i \leq n} \langle x, v_{i} \rangle v_{i}$, where $(v_{i})_{i = 1}^{n}$ is a eigenbasis for $A$. Now $A x = \sum_{1 \leq i \leq n} \lambda_{i} \langle x, v_{i} \rangle v_{i}$, so for instance

\begin{itemize}
\item $\langle A x, x \rangle = \sum_{1 \leq i \leq n} \lambda_{i} |\langle x, v_{i} \rangle|^{2}$. Thus the quadratic form is just a positive linear combination of eigenvalues.
\item $\|A x\|^{2} = \sum_{1 \leq i \leq n} \lambda_{i}^{2} |\langle x, v_{i} \rangle|^{2} \leq \left(\max_{1 \leq i \leq n} \lambda_{i}^2 \right) \|x\|^{2}$. Hence $\|A\| = \max_{1 \leq i \leq n} |\lambda_{i}|$.
\end{itemize}
...
The message is: if one wants products to preserve positivity, everything degenerates to $\R^{m}$, or to diagonal maps, which isn't all that exciting.
...
\begin{proof}
	...
	\end{lem}
	\begin{proof}
		Note that $\langle v, v \rangle = \rangle A v, v \rangle = \langle P_{v} v, v \rangle + \langle P_{w} v, v \rangle = \langle v, v \rangle + |\langle v, w \rangle|^2/\langle v, v\rangle$, so $\langle v, w \rangle = 0$.
	\end{proof}
	\begin{lem}
...
Proposition \ref{basic_positive} tells us that such order is indeed partial order on the $\R$-vector space of real maps. More explicitly, we have the following properties:

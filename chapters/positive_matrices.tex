...
If $(e_{i})_{i = 1}^{n}$ is any orthonormal basis of $V$, we see that if $A \in \H(V)$, for any $1 \leq i, j \leq n$ we have $\langle A e_{i}, e_{j} \rangle = \langle e_{i}, A e_{j} \rangle = \overline{\langle A e_{j}, e_{i} \rangle}$. But this means exactly that with respect to any orthonormal basis, matrix of a real map conjugate transpose of itself.

More generally, a matrix of adjoint, is simply a conjugate transpose of the matrix of the original map.
...
To this end, note that the quadratic form of $A + \lambda I$ at $v \in V$ is $\lambda (A + \lambda I) v, v \rangle = \langle A v, v \rangle + \lambda \langle v, v \rangle$. But if $\lambda \geq \|A\|$, the operator norm of $A$, the previous quantity is non-negative for any $v \in V$.
...
\begin{proof}
	...
	More generally, if it happens that for some $v \in V$ we have $\langle A v, v \rangle = 0$, we must have $v_{i} \perp v$ for any $1 \leq i \leq m$. But this means that were there such representation, we should have the following.

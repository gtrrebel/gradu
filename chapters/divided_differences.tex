...
As in the case $n = 1$, if for $n > 1$ we can continuously extend divided differences to the set $D_{n + 1}$, we should do that; we identify the resulting function with the original one. We will later proof that, as expected, this can be done, if and only $f \in C^{n}(a, b)$. In this case by \ref{mean_value} the extesion satisfies
\begin{align*}
...
\begin{kor}[General mean value theorem for divided differences]
	...
	\begin{align*}
		\min_{0 \leq i \leq n}(x_{i}) \leq \xi \geq \max_{0 \leq i \leq n}(x_{i})
	\end{align*}
	such that
	\begin{align}
		[x_{0}, x_{1}, \ldots, x_{n}] = \frac{f^{(n)}(\xi)}{n!}.

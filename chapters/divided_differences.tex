...
The real power of this approach comes with larger $k$. What about the case $k = 2$? Again, we already know the perfect answer: $2$-tone functions should be the convex functions.
...
We call \ref{nesting_rule} the \textit{nesting property} of divided differences. Although the analogy isn't perfect, one could think that this identity says that $m$'th derivative of the $n$'th derivative is the $(n + m)$'th derivative. 
...
The weight \ref{peano_weight} is called \textit{Peano kernel} (of order $n$, whenever there are $(n + 1)$ points). The points $x_{0}, x_{1}, \ldots, x_{n}$ are called the \text{nodes} of $w$.
...
They key idea in the proof was to split the test functions to two parts, one supported on $(a, b)$ and one on $(c, d)$. One cannot almost do the same with Peano Kernels, but larger the order $k$, the more parts we need. To closed mimic the proof we instead look at a generalization of Peano kernels, called splines.

\begin{maar}
	Let $k \in \Z_{+}$ nd $(a, b)$ an open interval. We say that $f : (a, b) \to \R$ is \textbf{spline} of order $k$ (on $(a, b)$) if there exists non-negative integer $N$, and points $a < x_{1} < x_{2} < \ldots < x_{N} < b$ (nodes of $f$) such that
	\begin{enumerate}
		\item $f \in C^{k - 2}(a, b)$ (this is void for $k = 1$)
		\item $f$ is polynomial of degree less than $k$ on intervals $(x_{1}, x_{2}), (x_{2}, x_{3}), \ldots, (x_{N - 1}, x_{N})$.
		\item $f(x) = 0$ for $a < x < x_{1}$ and $x_{N} < x < b$.
		\item $f$ is not polynomial on any neighbourhood of the points $x_{1}, x_{2}, \ldots, x_{N}$.
	\end{enumerate}
	We also say that a spline is positive if it is non-negative (function).
\end{maar}

We denote the (positive) splines of order $k$ on interval $(a, b)$ by ($S_{+}^{k}(a, b)$) $S^{k}(a, b)$.

\begin{lause}
	Let $k \in \Z_{+}$ and $(a, b)$ an open interval. Then
	\begin{enumerate}[(i)]
		\item If $f \in S^{k}(a, b)$ and $x_{1}, x_{2}, \ldots, x_{N}$ are the nodes of $f$, then there exists non-zero constants $t_{1}, t_{2}, \ldots, t_{N}$ such that
		\begin{align*}
			f(x) = \sum_{i = 1}^{N} t_{i} (x - x_{i})_{+}^{k - 1}
		\end{align*}
		for any $x \in (a, b) \setminus \{x_{1}, x_{2}, \ldots, x_{N}\}$.
		\item If $f \in S^{k}(a, b)$ and $f \neq 0$, then $f$ has at least $k + 1$ nodes.
		\item $S^{k}(a, b) = \vspan \{ f : (a, b) \to \R | \text{$f$ is a Peano Kernel on $(a,b)$}\}$
		\item $S_{+}^{k}(a, b) = \text{cone} \{ f : (a, b) \to \R | \text{$f$ is a Peano Kernel on $(a,b)$}\}$
	\end{enumerate}
\end{lause}
\begin{proof}
	\begin{enumerate}[(i)]
...
The previous result implies that for $k \in \Z_{+}$ and open interval $(a, b)$ we have a unique dual pairing $\langle \cdot, \cdot \rangle_{S}$ between $\R^{(a, b)}$ and $S^{k}(a, b)$, satisfying
\begin{align*}
	\langle f, w_{x_{0}, x_{1}, \ldots, x_{k}} \rangle_{S} = [x_{0}, x_{1}, \ldots, x_{k}]_{f}
\end{align*}
for any $f \in \R^{(a, b)}$ and $a < x_{0} < x_{1} < \ldots < x_{k} < b$.

\begin{lem}\label{spline_splitting_theorem}
	Let $a < c < b < d$ be reals, $k \in Z_{+}$ and $f \in S^{k}_{+}(a, b)$. Then there exists $f_{1}, f_{2} \in S^{k}_{+}(a, b)$ such that $\supp(f_{1}) \subset (a, b)$ and $\supp(f_{2}) \subset (c, d)$. 
\end{lem}
...
Now we are ready to translate the proof.
...
\end{prop}
\begin{proof}
	This follows immediately from lemma \ref{spline_splitting_theorem}.
\end{proof}
...
\section{Analyticity and Bernstein's theorem}
...
\begin{lause}[Bernstein's theorem]

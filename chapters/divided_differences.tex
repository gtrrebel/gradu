...
If $f$ is sufficiently smooth, we should also define $[x, x]_{f} = f'(x)$: if $f \in C^{1}(\R)$, this gives continuous extension to $[\cdot, \cdot]_{f}$. Much of the power of divided difference comes however from the fact that they conveniently carry same information even if we do not do such extension.
...
The idea of higher order divided differences is to push this interplay between (usual) divided difference and derivative to higher orders.
...
Why might it be desirable to make such definition, replace higher order derivatives with this mess of a expression? The key idea is that when one talks about high order differentiability, one has to be reasonably careful with everthing: even if functions converge pointwise, same need not be true for derivatives, let alone higher order ones. Higher order derivatives are rather fragile. If one can however translate derivatives to linear combinations, one doesn't have to worry that much anymore.
...
We defined divided differences only for real valued functions, but codomain could just as well any real or complex vector space. it's not very clear why such definition should correspond to anything useful.

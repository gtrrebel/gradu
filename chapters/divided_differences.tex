...
Finally, we can relate divided differences to divided differences of the derivative.

\begin{prop}
	If $f \in C^{1}(a, b)$ and $a < x_{1}, x_{2}, \ldots, x_{n} < b$ then
	\[
...
Actually we can prove a slightly better statement. Let $1 \leq n, n$ and denote
\[
...
\begin{lause}\label{bounded_div}
	...
	\[
		1 \leq \frac{\Lip(f^{(n - 1)})}{\sup_{a < x_{0} < x_{1} < \ldots < x_{n}} |[x_{0}, x_{1}, \ldots, x_{n}]_{f}|} \leq n!.
	\]
\end{lause}
...
\begin{proof}
	...
	\]
	Letting $x' \to x$ and $y' \to y$ we see that $f \in C^{1}(a, b)$ and $f'$ is Lipschitz.
	...
	\[
		[x_{1}, x_{2}, \ldots, x_{n}]_{f'} = \sum_{1 \leq i \leq n} [x_{1}, x_{2}, \ldots, x_{i - 1}, x_{i}, x_{i}, x_{i + 1}, \ldots, x_{n}]_{f}
	\]
	$f'$ has bounded $(n - 1)$:th divided differences, and by the induction hypothesis, $f \in C^{(n - 1)}(a, b)$ and $f^{(n - 1)}$ is Lipschitz. Induction also immediately gives the right inequality.
	...
	\begin{align*}
		...
		&= \left|\frac{f^{(n - 1)}(\xi_{1}) - f^{(n - 1)}(\xi_{2})}{x_{0} - x_{n}} \right| \\
		&\leq \Lip(f^{(n - 1)}) \left|\frac{\xi_{1} - \xi_{2}}{x_{0} - x_{n}}\right|
	\end{align*}
	for some $x_{0} \leq \xi_{1} \leq \xi_{2} \leq x_{n}$. Hence $n$:th divided difference is bounded, and we also have the left inequality.
\end{proof}
...
\begin{lause}
	Let $f : (a, b) \to \R$ and $k \geq 2$. If $f$ is $k$-tone, then $f \in C^{k - 2}(a, b)$, $f^{(k - 2)}(x)$ is convex and $f^{(k)}(x)$ is exists and is non-negative almost everywhere.
\end{lause}

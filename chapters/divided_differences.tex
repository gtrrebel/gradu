...
\begin{proof}
	...
	\begin{align*}
		[\cdot, \cdot, \ldots, \cdot]_{f} = 0 \Leftrightarrow \text{ $f$ is a polynomial of degree less $k$}.
	\end{align*}
...
\begin{align*}
	x \mapsto \frac{1}{2 \pi i} \int_{\gamma} \frac{p_{X}(x) - p_{X}(z)}{x - z}\frac{f(z)}{p_{X}(z)} dz = [x_{0}, x_{1}, \ldots, x_{n}]_{f [x, \cdot]_{p_{X}}}.
\end{align*}
More generally, if some of the points coincide, we get the Hermite interpolation polynomial.
\end{esim}

...
\begin{itemize}
	...
	\item $P_{n}(a, b)$, the cone of $n$-monotone functions (on $(a, b)$), was defined as dual cone of maps of the form $f \mapsto \langle (f(B) - f(A)) v, v \rangle$, were $v \in V$ and $A, B \in \H_{(a, b)}$ are such that $A \leq B$.
\end{itemize}
...
\end{align*}
Since $\beta$ can be any real, we should have $\sum_{i = 1}^{m} c_{i} = 0$. Choosing $\lambda \in \R$, $\mu = \delta_{\lambda}$, $\alpha = 0$ and writing $r(\lambda) =  \sum_{i = 1}^{m} c_{i}/(\lambda - z_{i})$, we see that
\begin{align*}
...
Let us first make couple of reality checks concering the sets $R(X)$, $R_{\pm}(X)$ and $R_{+}(X)$. For rational function $r$, denote by $r^{*}$ the rational function for which
\begin{align*}
...
We will mostly study the case $X = \Hp \cup \Hm$: notice that $R(\Hp \cup \Hm)$ generalizes the rational functions in both the complex and real settings. How do they define linear functionals? If we are given any $r \in R(\Hp \cup \Hm)$, write $r$ in the form $\sum_{i = 1}^{m} c_{i} (\cdot - z_{i})^{-1}$, where $m \geq 0$, $c_{1}, c_{2}, \ldots, c_{m} \in \C$ and $z_{1}, z_{2}, \ldots, z_{m} \in \Hp \cup \Hm$. Then, if we are given any $\varphi \in \C^{\Hp}$, we set
\begin{align*}
	\varphi \mapsto \sum_{i = 1}^{m} c_{i} \varphi(z_{i}).
\end{align*}
...
\end{align*}
for any Pick function $\varphi$? One could use Theorem \ref{pick_nevanlinna_herglotz_representation_theorem} to prove that these functionals are exactly the elements of $R_{+}(\Hp)$, but as we haven't proven \ref{pick_nevanlinna_herglotz_representation_theorem}, we will give alternate argument.
...
\begin{proof}
	...
	\end{align*}
	As $|\cdot - z|^{-2} \in R_{\pm}(\Hp \cup \Hm)$, at least $\varphi(\overline{z}) - \overline{\varphi(z)} \in i \R$ for any $z \in \Hp$.
	...
	\end{align*}
	Now if that would imply that $\langle \varphi, r_{z, c} \rangle_{L} \in \R$, we would be done. Indeed, it is easy to see that
	\begin{align*}
...
By the previous lemma, any $\varphi \in \pickclass(\Hp \cup \Hm)$ can be translated to satisfy (i). We denote the set of such translates by $\pickclass(\Hp)$.
...
\begin{proof}
	...
	\end{align*}
	It should be noted that $\|\cdot\|_{R}$ is not norm, as it is infinite for some elements of $R(\Hp \cup \Hm)$. It will nevertheless define an topology and one straightforwardly checks that
	\begin{align*}
...
Let us sketch alternate and more conceptual argument for Lemma \ref{weak_analytic}.
...
\begin{proof}
	...
	\end{align}
	we are done. Indeed: we have then found $(\cdot - w)^{-k - 1}$ an approximation (in the sense of \ref{rat_approx}) by a rational function with poles of lower order, and by induction hypothesis this lower order approximation has approximation with simple poles, as desired.
	...
	\begin{align*}
		...
		\leq & |z - w|\sum_{i = 1}^{k} \frac{1}{\Im(w)^{k + 1 - i}} \sum_{j = 1}^{i} \left\|\frac{1}{(\cdot - z)^{j} (\cdot - w)^{i + 1 - j}}\right\| \\
		\leq & |z - w| \left\|\frac{1}{(\cdot - z)(\cdot - w)}\right\|\sum_{i = 1}^{k} \frac{1}{\Im(w)^{k + 1 - i}} \sum_{j = 1}^{i} \frac{1}{\Im(z)^{j - 1} \Im(w)^{i - j}} \\
		= & O(|z - w|)
	\end{align*}
	for $z, w$ in any compact $K \subset \Hp$.

	If $r \in R^{\infty}_{\pm}(\Hp \cup \Hm)$, then also the approximation we constructed satisfies $s \in R_{\pm}(\Hp \cup \Hm)$.
...
\begin{proof}[Proof of Proposition \ref{extension_positivity}]
	t.f.i.f. \ref{rat_approx}.
\end{proof}

The point of all of this is that these arguments can be ``reversed".
...
All of the arguments of the section ``step $2$" extend almost verbatim for sets $\pickclass(X \cup X^{*})$:
\begin{enumerate}[(i)]
...
\end{align*}
as the  ``norm" and everything works as expected. Also, the ``norms" $\|\cdot\|_{R, z_{0}}$ are all equivalent anyway, as one checks easily enough.
...
\begin{huom}\label{taylor_section_rem}
	...
	\begin{lause}\label{pick_taylor_sections}
		...
		is positive.
	\end{lause}
	Here the definition of $\langle \varphi, \cdot\rangle_{L}$ is completed exactly how one would expect. Theorem \ref{pick_taylor_sections} could be proved with pretty much same ideas as Theorem \ref{pick_interpolation}; we're not going to do that, however.
\end{huom}

...
So there are no non-trivial $\C$-linear functionals on $\C^{\Hp}$, non-negative on Pick functions.
...
\end{align*}
should be non-negative. We will show in short while that in this case also $\sum_{i = 1}^{m} \left(c_{i} z_{i} + \overline{c_{i}} \overline{z_{i}}\right) \geq 0$.

These considerations suggest that elements of $C^{*}$ should correspond to non-negative rational functions, whereas elements of $(\C^{\Hp})^{*}$ should be rational functions mapping reals to reals. One could work with such functionals, but it is convenient to broaden one's view slightly: we will consider also complex rational functions. We will specifically consider the following family of rational functions.

\begin{maar}
	We will denote by $R(\Hp)$ the $\C$-vector space of rational functions $r$ such that:
	\begin{itemize}
		\item $r$ vanishes at infinity.
		\item All poles of $r$ lie in $\Hp \cup \Hm$.
		\item All poles of $r$ are simple.
	\end{itemize}
	We will also write $R_{+}(\Hp)/R_{\pm}(\Hp)$ for the functions in $R(\Hp)$, which are non-negative/real on $\R$.
\end{maar}

Let us first make couple of reality checks concering the sets $R(\Hp)$, $R_{\pm}(\Hp)$ and $R_{+}(\Hp)$. For rational function $r$, denote by $r^{*}$ the rational function for which
\begin{align*}
...
\begin{prop}\label{pick_functional_span}
	\begin{enumerate}[(i)]
		\item
		\begin{align*}
			R(\Hp) = \vspan_{\C} \left\{\frac{1}{\cdot - z} | z \in \Hp \cup \Hm \right\}.
		\end{align*}
		\item
		\begin{align*}
			R_{\pm}(\Hp) = \vspan_{\R} \left\{\frac{c}{\cdot - z} + \frac{\overline{c}}{\cdot - \overline{z}} | c \in \C, z \in \Hp \right\} = \{r + r^{*} | r \in R(\Hp) \}.
		\end{align*}
		\item
		\begin{align*}
			R_{+}(\Hp) = \left\{ r r^{*} | r \in R(\Hp) \right\}.
		\end{align*}
		\item
		\begin{align*}
			R(\Hp) = \vspan_{\C} R_{\pm}(\Hp)
		\end{align*}
		\item
		\begin{align*}
			R_{\pm}(\Hp) = \vspan_{\R} R_{+}(\Hp) = \{r_{1} - r_{2} | r_{1}, r_{2} \in R_{+}(\Hp)\}
		\end{align*}
	\end{enumerate}
\end{prop}
\begin{proof}
...
Notice that the set $R(\Hp)$ generalizes the rational functions in both the complex and real settings. How do they define linear functionals? If we are given any $r \in R(\Hp)$, write $r$ in the form $\sum_{i = 1}^{m} c_{i} (\cdot - z_{i})^{-1}$, where $m \geq 0$, $c_{1}, c_{2}, \ldots, c_{m} \in \C$ and $z_{1}, z_{2}, \ldots, z_{m} \in \Hp \cup \Hm$. Then, if we are given any $\varphi \in \C^{\Hp}$, we set
\begin{align*}
	\varphi \mapsto \sum_{i = 1}^{m} c_{i} \varphi(z_{i}).
\end{align*}
Except now there's a slight problem: if $z_{i} \in \Hm$, $\varphi(z_{i})$ is not defined! For Pick functions such rational functions correponded to conjugate evaluations, so we should of course interpret the above definition with the following abuse:
\begin{align}\label{abuse}
	\varphi(\overline{z}) = \overline{\varphi(z)},
\end{align}
for any $z \in \Hp$. Instead, we do something else. Note that every function in $\C^{\Hp}$ can be interpreted as a function in $\C^{\Hp \cup \Hm}$, extending with \ref{abuse}. In this way every element of $R(\Hp)$ corresponds to a $\C$-linear functional $\C^{\Hp \cup \Hm} \to \C$. It is easy to see that every such functional also corresponds to an element in $R(\Hp)$. We hence have an dual pairing between $\C^{\Hp \cup \Hm}$ and $R(\Hp)$, $\C$-linear in both arguments, denoted by $\langle \cdot, \cdot \rangle_{L}$.

To recap $\langle \cdot, \cdot \rangle_{L} : \C^{\Hp \cup \Hm} \times R(\Hp) \to \C$ is the unique $\C$-bilinear mapping for which
\begin{align*}
	\langle \varphi, (\cdot - z)^{-1}\rangle_{L} = \varphi(z)
\end{align*}
for any $\varphi \in \C^{\Hp \cup \Hm}$ and $z \in \Hp \cup \Hm$. Note also that for fixed $r \in R(X)$, the map $\varphi \mapsto \langle \varphi, r\rangle_{L}$ is continuous. This pairing is of course essentially the same pairing we defined in section \ref{dual_pairing_section}.

So finally: which elements in $r \in R(\Hp)$ are such that
\begin{align*}
	\langle \varphi, r \rangle_{L} \geq 0
\end{align*}
for any Pick function $\varphi$? Theorem \ref{pick_nevanlinna_herglotz_representation_theorem} implies that these functionals are exactly the elements of $R_{+}(\Hp)$, but as we haven't proven \ref{pick_nevanlinna_herglotz_representation_theorem}, we will give alternate argument.
...
\begin{lause}\label{pick_functionals}
	...
	\begin{align*}
		\langle \varphi, r \rangle_{L} \geq 0
	\end{align*}
	for any $\varphi \in \pickclass$, if and only if $r \in R_{+}(\Hp)$.
\end{lause}
\begin{proof}
	...
	\end{align*}
	for any $\lambda \in \R$ and $r \in R(\Hp)$. We must hence have $r \in R_{+}(\Hp)$.

	Conversely, fix any $r \in R_{+}(\Hp)$. We already know that $\langle \varphi_{\lambda}, r\rangle \geq 0$ for any $\lambda \in \R$ so by continuity it suffices to prove the following result.

	\begin{lem}\label{pick_dense}
		Under the topology of pointwise convergence one has
		\begin{align*}
			\pickclass \subset \overline{\cone}\{\varphi_{\lambda} | \lambda \in \R\} \subset \C^{\Hp} \subset \C^{\Hp \cup \Hm}.
		\end{align*}
	\end{lem}
	\begin{proof}
...
We will call the maps $\varphi \mapsto \langle \varphi, r\rangle_{L}$ for fixed $r \in R_{+}(X)$ \textbf{Pick functionals}.

One can write the Pick functionals also in terms of divided differences. If $r \in R_{+}(\Hp)$, we have
\begin{align*}
...
Recall that our aim is to prove that $\pickclass \in \C^{\Hp \cup \Hm}$ is dual cone of $R(\Hp)$ in the sense of $\langle \cdot, \cdot \rangle_{L}$. In other words: we should prove that if $\varphi \in \C^{\Hp \cup \Hm}$ is such that
\begin{align}\label{weakly_pick_def}
	\langle \varphi, r\rangle_{L} \geq 0 \text{ for any  $r \in R_{+}(\Hp)$},
\end{align}
...
\end{enumerate}
Note that if $\varphi$ is analytic on $\Hp$, it is clearly also analytic on $\Hm$.

We will call a function $\varphi \in \C^{\Hp \cup \Hm}$ satisfying \ref{weakly_pick_def} \textbf{weakly Pick functions}, and denote the set of weakly Pick functions by $\pickclass(\Hp)$. It's not very hard to show that any weakly Pick function satisfies $(i)$ and $(ii)$. Indeed, by Proposition \ref{pick_functional_span} $\langle \varphi, r\rangle_{L} \in \R$ for any $\varphi \in \pickclass(\Hp)$ and $r \in R_{\pm}(\Hp)$. In particular
\begin{align*}
	c \varphi(z) + \overline{c} \varphi(\overline{z}) = \langle \varphi, c (\cdot - z)^{-1} + \overline{c} (\cdot - \overline{z})^{-1}\rangle_{L} \in \R,
\end{align*}
for any $c \in \C$ and $z \in \Hp$, which easily implies $(i)$. Note that the first property implies that any weakly Pick function can be interpreted as a function in $\C^{\Hp}$. For $(ii)$, note that for any $z \in \Hp$ we have $|\cdot - z|^{-2} \in R_{+}(\Hp)$, so
\begin{align*}
	0 &\leq \langle \varphi, |\cdot - z|^{-2}\rangle_{L} \\
	&= \left\langle \varphi, \frac{1}{z - \overline{z}}\frac{1}{\cdot - z} + \frac{1}{\overline{z} - z}\frac{1}{\cdot - \overline{z}}\right\rangle_{L} \\
	&= \frac{\varphi(z) - \varphi(\overline{z})}{z - \overline{z}} \\
	&= \frac{\Im(\varphi(z))}{\Im(z)},
\end{align*}
as desired.
...
\begin{proof}
	...
	\end{align*}
	The function $r_{z, w} := (\cdot - z)^{-1} - (\cdot - w)^{-1}$ tends to zero (in some sense) as $z \to w$, so something like this ought to be true: $R(\Hp)$ should have topology for which this convergence happens and the map $r \mapsto \langle \varphi, r\rangle$ is continuous.
...
\begin{lem}
	Let $\varphi \in \pickclass(\Hp)$. Then $\varphi$ is analytic.
\end{lem}
\begin{proof}
	Note that if we manage to show that the order $2$ divided differences of $\varphi$ are locally bounded, Theorem \ref{bounded_div} implies the claim. Strictly speaking we only proved Theorem \ref{bounded_div} on real line, but the proof would be almost identical in the complex case. Observe that for any $z_{0}, z_{1}, z_{2} \in \Hp$ we have
	\begin{align*}
		[z_{0}, z_{1}, z_{2}]_{\varphi} = \left\langle \varphi, \frac{1}{(\lambda - z_{0}) (\lambda - z_{1}) (\lambda - z_{2})}\right\rangle_{L}.
	\end{align*}
...
In similar vein one could cook up estimates for higher order divided differences.

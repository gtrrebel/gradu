...
Before the proof we need an interesting fact\footnote{We don't really need, but it is an interesting fact nevertheless.}.

\begin{lem}\label{disc_lemma}
	Let $\mathcal{F}$ be a non-empty family of (non-empty) closed discs such that for any $\D_{1}, \D_{2} \in \mathcal{F}$ we have $\mathcal{F} \ni \D_{3} \subset \D_{1} \cap \D_{2}$. Then
	\begin{align*}
		\bigcap_{\D \in \mathcal{F}} \D
	\end{align*}

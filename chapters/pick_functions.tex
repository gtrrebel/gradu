...
\begin{lem}\label{disc_lemma}
	Let $\mathcal{D}$ be a non-empty family of (non-empty) closed discs such that for any $\D_{1}, \D_{2} \in \mathcal{D}$ we have $\mathcal{D} \ni \D_{3} \subset \D_{1} \cap \D_{2}$. Then
	\begin{align*}
...
\end{lem}
\begin{proof}
	Denote the intersection by $K$. Since closed discs are compact and convex, so is $K$. Additionally, since the family has finite intersection property, $K$ is also non-empty. By compactness one may take two points on $K$ with largest distance. If the points are equal, the intersection is a single points, and we are done. If not, without loss of generality we may assume that the points are $-1$ and $1$. Now fix $\varepsilon \in (0, \frac{1}{4})$.

	We claim that there exists a disc $\D \in \mathcal{D}$ such that $\D$ is contained in the strip $S_{\varepsilon} := \{z \in \C | |\Re(z)| < 1 + \varepsilon\}$. For this note that for any $M > 0$ the family $\{[1 + \varepsilon - i M, 1 + \varepsilon + i M] \cap \D | \D \in \mathcal{D}\}$ (where $[1 + \varepsilon - i M, 1 + \varepsilon + i M]$ denotes the closed interval from $1 + \varepsilon - i M$ to $1 + \varepsilon + i M$) is family of compact sets with empty intersection, so they have finite subfamily with empty intersection. By the main condition if follows that we find a disc in $\mathcal{D}$ with empty intersection with the interval. Taking $M$ large enough we see that one of the discs has empty intesection with the line $\{z \in \C | \Re(z) = 1 + \varepsilon\}$. Playing the same game with $-1 - \varepsilon$ and taking the intersection we find the required disc.

	Now we claim that any disc in the family contains the disc $\D(0, 1 - 2 \sqrt{\varepsilon})$. To show this note that every disc has radius at least $1$ so it is sufficient to prove that any disc has center in $\D(0, \sqrt{\varepsilon})$. It is sufficient to consider discs in $S_{\varepsilon}$. If $\D_{0}$ is such disc let $z_{0}$ be it's center since $2 + 8 \varepsilon \geq 2(1 + \varepsilon)^{2} \geq |1 - z_{0}|^2 + |1 + z_{0}|^2 = 2 + 2 |z_{0}|^2$, from which the second claim follows.

	Since $\varepsilon$ was arbitrary it follows that $K$ (being compact) contains the closed disc $\overline{\D}(0, 1)$. But since $K$ has diameter $2$, it can't contain anything else, so $K = \overline{\D}(0, 1)$ and we are done.
...
\end{lause}
\begin{proof}
	Note that respective claim holds for Schur functions by Montel's theorem, so hence also holds for Pick functions $\ldots$ with one small caveat: the Schur function constant $-1$ corresponds to constant $\infty$, which isn't proper Pick function. But by the boundedness condition this cannot happen.
\end{proof}

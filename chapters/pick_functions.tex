...
\begin{proof}
	...
	\begin{align*}
		...
		\end{cases}
	\end{align*}
	Using the trivial inequality $\binom{2 n - 2 - j}{n - 1} \leq 2^{2 n - 2 - j}$ one easily sees that $\|A_{j}\| \leq 2^{-j}|b_{j}|$ for any $0 \leq j \leq n - 1$. Additionally it is easy to verify that $A_{n - 1}$ negative eigenvalue with absolute value at least $2^{- n - 2}|b_{n - 1}|$.
